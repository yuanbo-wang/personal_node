vue中使用节流函数  取不到this的问题   需要改变方法的写法 
通过
``` js
  methods:{
    throttle:_throttle(fn1,waittime,options)
  }
...
_throttle(func, wait, options) {
  ...
  var timeout = null // 上一次触发事件回调的时间戳。 默认为 0 是为了方便第一次触发默认立即执行
  var previous = 0 // 如果没有传入 options 参数 // 则将 options 参数置为空对象
  if (!options) { options = {} }
  var later = function() {
